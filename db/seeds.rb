...
5.times { User.create!({name: FFaker::Name.unique.name}) }
...
10.times { Event.create!({cause: causes.sample, style: styles.sample, location: Faker::Address.street_address, date: Faker::Date.between(Date.parse('04/01/2019'), Date.parse('15/02/2019')), time: Faker::Time.forward(1, :day).strftime('%H:00')}) }
